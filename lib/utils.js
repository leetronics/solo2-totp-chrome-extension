// lib/utils.js
// Utility functions for SoloKeys Vault extension
...
export function isHiddenCredential(credential) {
    const passwordPrefix = '__solo_pw__:';
    const rawName = String(credential?.rawName || credential?.name || '').trim().toLowerCase();
    const normalizedName = rawName.startsWith(passwordPrefix)
        ? rawName.slice(passwordPrefix.length)
        : rawName;

    return normalizedName.startsWith('hmacslot');
}

export function filterVisibleCredentials(credentials) {
    if (!Array.isArray(credentials)) {
        return [];
    }

    return credentials.filter(credential => !isHiddenCredential(credential));
}
